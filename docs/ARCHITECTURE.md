...
4. Committed text goes through a private virtual keyboard with its own
   ...
   Injection below the toolkit is what lets every application receive
   ordinary key events, X11 clients through Xwayland included, without
   cooperating and without a framework in between. The price is that a
   compositor without `zwp_virtual_keyboard_v1` cannot run the engine at all;
   the startup check reports that and stops rather than degrading. `TextSink`
   stays an abstraction for the platform backends still to come (X11, macOS).
